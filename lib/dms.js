...
class DmsModel {
  ...
  async getOrganizations(opts={}, standard=true) {
    const action = 'organization_list'
    let url = new URL(resolve(this.api, action))
    const params = Object.assign({}, {all_fields: true, sort: "package_count"}, opts)
    let response = await fetch(url, {
      method: 'POST',
      body: JSON.stringify(params),
      headers: { 'Content-Type': 'application/json' }
    ...
    if (standard) {
      return response.result.map(org => {
  ...
  async getCollection(collection, opts, standard=true) {
    const action = 'group_show'
    let url = new URL(resolve(this.api, action))
    const params = Object.assign({}, {id: collection}, opts)
    console.log(params)
    let response = await fetch(url, {
      method: 'POST',
      body: JSON.stringify(params),
    ...
    response = await response.json()
    if (standard) return utils.convertToStandardCollection(response.result)
    return response.result
  }
}
